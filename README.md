...
| LLM 决策可复现 | 按 (model, prompt, params, seed) 缓存 | ✅ |
...
| **v0.7.0** | **当前** | **金融场景增强：✅ HMAC 哈希链审计、✅ 幂等性（Idempotency-Key + 跨进程锁）、✅ HTTP 限流/重试、✅ LLM 决策可复现缓存、✅ 配额持久化+多租户、✅ trace 脱敏（JWT/私钥）、✅ WAL 崩溃恢复** |
...
已具备核心金融能力（审计/幂等/限流/脱敏/可复现），适用于只读分析和受控写入场景。
