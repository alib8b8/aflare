...
### Changed
- **主 usage 补齐 10 个可达命令**：PrintUsage 此前只列 18 个命令，`validate` / `list` / `init` / `config` / `secrets` / `schedule` / `audit` / `review` / `serve` / `webui` 均已实现且可分发，但用户从 help 不可发现——现全部补入（中/英/俄三语 locale 新增 9 个 usage.* key；`usage.schedule` key 早已存在只是从未被引用）；同步删除 5 个引用不存在命令的死 key（usage.debug / usage.history / usage.plugin / usage.secret / usage.template，×3 locale）
...
- **俄语 locale 缺 8 个 resume.* key（本轮全量自检发现）**：`resume.completed/failed/list_empty/list_title/load_failed/not_paused/resuming/usage` 只在中/英 locale 有——俄语用户执行 `aflare resume` 时界面直接显示 raw key（如 `resume.list_title`）；补齐 8 个俄语翻译，三语 key 集合现已完全一致（172 个）
