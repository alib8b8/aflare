...
- **codex_agent 节点（补账，PR #78 已合入 main 但此前无 CHANGELOG 记录）**：新增 `internal/nodes/codex.go`——工作流步骤委托 `codex exec` 执行（prompt 为单一 argv 元素、绝不经过 shell，model 值经白名单校验阻断 argv 级把戏，cwd 规范化后经路径校验）；配套 ReAct agent 会话上下文预算压缩与 pause-resume 状态持久化（CLI 展示 resume 提示）
