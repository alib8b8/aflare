...
- **所有者 git 身份落定**：今后提交统一使用 GitHub 已验证登记的 Gmail 身份（`sjxj19921205@gmail.com`；历史 0 提交，2026-09 起启用，避免再新增匿名身份）；`.mailmap` 规范身份改为该邮箱并映射两个历史 noreply 拼写（仅影响本地 `git log` 显示，不重写历史，GitHub 网页端显示不受影响）；PROVENANCE.md §2 身份表新增该条目（历史提交身份不变，仍待所有者逐项确认）
