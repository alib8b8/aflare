...
**Extensible ecosystem** — Custom nodes (Go), MCP Server / Client (`aflare mcp install` for built-in community servers), plugin system (community `.so`).
...
The price-monitoring workflows generated by `aflare create` cover four markets out of the box:
...
Other open data sources (East Money, Sina Finance public quote APIs, etc.) plug in directly through the `http_request` node, covering A-shares / HK / US stocks / funds — review their terms of service before use.

What aflare can and cannot do in finance:

| Use case | Feasibility | Notes |
|----------|-------------|-------|
| Price monitoring / threshold alerts | ✅ Fully supported | schedule → fetch → condition → notify, generated by `aflare create` in one sentence |
