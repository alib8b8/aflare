...
func TestParseArgs_HelpFlags(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			cmd, _, _, _, _, _ := ParseArgs(tc.args)
			if cmd != tc.expected {
...
func TestParseArgs_DryShort(t *testing.T) {
	cmd, args, _, dryRun, _, _ := ParseArgs([]string{"--dry", "run", "workflow.yaml"})
...
func TestParseArgs_MultipleFlags(t *testing.T) {
	cmd, args, safeMode, dryRun, mcpServer, lang := ParseArgs([]string{
...
func TestParseArgs_LangAtEnd(t *testing.T) {
	cmd, args, _, _, _, lang := ParseArgs([]string{"run", "workflow.yaml", "--lang", "en"})
...
func TestParseArgs_LangInvalidValue(t *testing.T) {
	_, _, _, _, _, lang := ParseArgs([]string{"--lang", "invalid-lang", "run", "workflow.yaml"})
...
func TestParseArgs_LangNoValue(t *testing.T) {
	cmd, args, _, _, _, lang := ParseArgs([]string{"--lang", "run", "workflow.yaml"})
