...
func NewUpgradeEngine(config *UpgradeConfig) *UpgradeEngine {
	return &UpgradeEngine{
		config:   config,
		state:    &UpgradeState{CurrentVersion: meta.GetVersion()},
...
func (e *UpgradeEngine) CheckAndUpgrade(ctx context.Context) {
	...
	if !meta.HasUpdate(meta.GetVersion(), release) {
		logger.Info("no update available", "current", meta.GetVersion(), "latest", release.TagName)
		return
	}

	logger.Info("update available", "current", meta.GetVersion(), "latest", release.TagName)
...
func (e *UpgradeEngine) PerformUpgrade(ctx context.Context, release *meta.GitHubRelease) {
	e.mu.Lock()
...
func copyFile(src, dst string) error {
	data, err := os.ReadFile(src) // #nosec G304 -- internally generated executable path
	if err != nil {
		return err
	}
...
func attemptAutoMerge(branch string) error {
	...
	cmd = exec.Command("git", "checkout", "--", branch) // #nosec G204 -- branch is internally generated by git workflow
	cmd.Dir = repoDir
	if err := cmd.Run(); err != nil {
	...
	cmd = exec.Command("git", "merge", "--no-ff", "--", branch) // #nosec G204 -- branch is internally generated by git workflow
	cmd.Dir = repoDir
	if err := cmd.Run(); err != nil {
