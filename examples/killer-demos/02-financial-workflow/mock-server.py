#!/usr/bin/env python3
"""Mock bank API for the financial-workflow killer demo.

模拟企业跨行转账涉及的多个端点，用于演示 Saga 事务补偿、限流、重试、幂等：

  POST /debit    付款行扣款（始终成功）
  POST /credit   收款行入账（amount=9999 强制失败，触发 saga 补偿）
  POST /refund   付款行退款（debit 的补偿）
  POST /reverse  收款行冲正（credit 的补偿）
...
# 账户余额（模拟）
_balances = {
    "CORP-ACC-001": 1000000.00,
    "CORP-ACC-002": 500000.00,
}
...
def main():
    print("=" * 60)
    print("  🏦  Mock Bank API — Financial Workflow Killer Demo")
    print("=" * 60)
    ...
    print("  Endpoints:")
    print("    POST /debit    — 扣款")
    print("    POST /credit   — 入账（amount=9999 触发失败）")
    print("    POST /refund   — 退款（debit 补偿）")
    print("    POST /reverse  — 冲正（credit 补偿）")
    print("    POST /notify   — 通知")
