...
**GitHub Actions（CI）**——把 aflare 工作流作为 CI 步骤运行。秒级安装预编译二进制（带校验和验证），无需 Docker 构建、无需编译：
...
支持 `set` 传参、`safe-mode`、`validate-only`，并暴露最终输出——详见 [action/README.md](action/README.md)。
...
> 生成的行情监控工作流数据来自公开行情接口（腾讯 / CoinGecko 等），可能有延迟，仅供个人研究参考，不构成投资建议。
...
```
L0: Agent        —  "监控我的 OpenFOAM 仿真，发散了告警我"
                    ├── ReAct 推理循环（思考 → 调工具 → 观察 → 回答）
                    └── 6 类可插拔能力（反思/HITL/效用驱动等）
                       ↓
L1: Workflow     —  YAML 确定性工作流（schedule → read_log → condition → notify）
                       ↓
L2: Runtime      —  确定性执行层
                    ├── DAG 并行调度
...
**可扩展生态** — 自定义节点（Go）、MCP Server / Client（`aflare mcp install` 一键安装内置社区 server）、插件系统（社区 `.so`）、[GitHub Action](action/README.md)（在 CI 中运行工作流，校验和验证的二进制安装）。
...
[`examples/real-world/`](examples/real-world/) 下即取即用的场景工作流——每个目录自带工作流 YAML、样例数据与 README：

- **工业监控** — [`openfoam-watchdog`](examples/real-world/openfoam-watchdog/) 监控运行中的 CFD 仿真是否发散：对求解器日志做确定性正则检测、if/else 分支、LLM 分诊笔记可降级——告警链路永不依赖模型可用性；[`log-similarity-rag`](examples/real-world/log-similarity-rag/) 为新故障检索最相似的历史案例（离线关键词匹配，或经 OpenAI 兼容 embeddings 端点做语义检索，如 vLLM 部署 WeMM-Embedding），让 LLM 分诊引用真实先例而非从零推理；[`log-anomaly-detector`](examples/real-world/log-anomaly-detector/) 过滤错误行并按严重模式分支
- **DevOps / CI** — 夜间 CI 修复、PR 评审门禁、代码评审流水线（与 [GitHub Action](action/README.md) 天然搭配）
- **调研与知识** — 多 Agent 调研、知识策展、MemHarness 记忆批判
- **批量处理等** — URL 批量处理、文档提取、TDD 循环

工业监控包演示了生产级 AI 的核心范式：**确定性检测在前、LLM 叙述在后、每个 LLM 步骤可降级**——告警链路扛得住模型故障，输出里的每个数字都来自确定性提取而非模型想象。
...
| **main** | **进行中** | GitHub Action（CI 集成）、真实场景工作流包（工业监控：OpenFOAM 看门狗 + 相似案例 RAG 分诊）、Agent 互联深化（更多通道与监督策略）、个人笔记软件连接器、国产芯片适配完善 |
| v1.0 | 计划中 | 稳定 API、LTS |
...
- [入门指南](docs/getting-started.md) · [教程](docs/tutorial.md) · [YAML 语法](docs/getting-started.md#step-2-create-your-first-workflow)
